...
//       return res.status(404).json({ error);
    //   // LINT: unreachable code removed} res.json({ user   });
  } catch(error)'
...
// const updatedUser = awaitUser.update(req.user.id, updates);'
...
// const userWithPassword = awaitUser.findByIdWithPassword(req.user.id);
      // Verify current password
// const isValid = awaitUser.verifyPassword(currentPassword, userWithPassword.password);
  if(!isValid) {
        // return res.status(401).json({ error);
    //   // LINT: unreachable code removed}
      // Update password
// const hashedPassword = awaitbcrypt.hash(newPassword, 10);
