...
try {'
  ...
  console.warn(' Git repository detected'); catch(/* _error */)'
...
try {'
  execSync('git config user.name', stdio); catch(/* _error */) {'
...
console.warn('---');
try {
  const result = execSync(agentCompleteCmd, { encoding);
  ...
  if(parsed.committed) {'
    console.warn('\n Latest Git Commit);''
    const latestCommit = execSync('git log -1 --pretty=format);'
