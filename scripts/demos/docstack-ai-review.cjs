...
async
            function analyzeDocumentWithAI(docType, service, docId, content, metadata) {
              ';
...
"
Focus on practical, actionable feedback. IMPORTANT: Respond
              with ONLY the
              JSON;
              object, no;
              other;
...
- Need to manage Redis high availability`,`
    metadata: {`
      ...
      tags: ['sessions', 'redis', 'scaling'] } }
              ...
              demoDoc.docType, demoDoc.service, demoDoc.docId, demoDoc.content, demoDoc.metadata;
              )
              // const aiAnalysis = awaitanalyzeDocumentWithAI(
              demoDoc.docType, demoDoc.service, demoDoc.docId, demoDoc.content, result.metadata;
              )
              if (aiAnalysis) {
                if (aiAnalysis.suggested_approvers?.length > 0) {
                  if (aiAnalysis.detected_issues?.length > 0) {
                    ...
                    if (aiAnalysis.improvement_suggestions?.length > 0) {
                      ...
                      demoDoc.docType, demoDoc.service, result.routing.approvers, demoDoc.content;
                      )
                      if (routingReview) {
