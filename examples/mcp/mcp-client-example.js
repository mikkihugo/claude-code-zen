...
class MCPClientExample {
  ...
  async completeWorkflowExample() {
    ...
    try {
      // 1. Initialize swarm
      console.warn('Step 1: Initializing swarm');
      await this.swarmInitExample();

      // 2. Spawn agents
      console.warn('Step 2: Spawning agents');
      await this.agentSpawnExample();

      // 3. Set up memory context
      console.warn('Step 3: Setting up memory context');
      await this.memoryOperationsExample();

      // 4. Orchestrate main task
      console.warn('Step 4: Orchestrating tasks');
      await this.taskOrchestrateExample();

      // 5. Monitor progress
      console.warn('Step 5: Monitoring swarm status');
      await this.swarmStatusExample();

      // 6. Analyze performance
      console.warn('Step 6: Analyzing performance');
      await this.performanceAnalysisExample();

      // 7. Run diagnostics
      console.warn('Step 7: Running system diagnostics');
