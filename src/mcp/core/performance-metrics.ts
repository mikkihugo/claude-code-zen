...
/** Record batch processing metrics */
   ...
   * @param {number}
        ...
        (this.metrics.batches.avgProcessingTime * (this.metrics.batches.total - 1) +
          processingTime) /
          this.metrics.batches.total;
        //   }
              ...
              updateMemoryMetrics((bufferSize = 0));
              {
                ...
                if (this.history.latencies.length > 0) {
                  ...
                  this.metrics.requests.p99Latency =
                    sortedLatencies[Math.floor(length * 0.99)] ?? 0;
                  //     }
                  //   }
   ...
   * @param {any}
                      ...
                      if (!this.history[type]) {
                        ...
                        if (this.history[type].length > this.maxHistoryLength) {
                          ...
                          this.history.timestamps = this.history.timestamps.slice(
...
/** Generate performance recommendations */
  ...
  if(summary.overview.errorRate > 0.1) {
    ...
    console.error(`[$new
                              Date().toISOString();
                              ] INFO [Metrics] Performance metrics reset`)
                              ``;
                              //   }
                              // }
                            }
