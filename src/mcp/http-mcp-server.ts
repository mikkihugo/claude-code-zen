...
//   super();
  this._config =
{port = = false,enableAllTools = = false,httpMode = = false,timeout = this._config.port!;
...
// {/g)
  ...
  if(!res.headersSent) {
      ...
      res.json({
        name => {
      try {
// const response = awaitthis._mcpServer.handleMessage({
          jsonrpc => {
      try {
// const response = awaitthis._mcpServer.handleMessage({
          jsonrpc => {
...
// const response = awaitthis._mcpServer.handleMessage({
    ...
    this._app.get('/mcp/tools', async(req => {'
      try {)
// const tools = awaitthis._mcpServer.toolsRegistry?.getAllTools()  ?? [];
        res.json({success = > ({
            name => {
      try {
...
// const tools = awaitthis._mcpServer.toolsRegistry?.getAllTools()  ?? [];
  ...
  if(err.code === 'EADDRINUSE''
          reject(new Error(`Port $this._portis already in use`));``
        } else {
          reject(err);
        //         }
      })});
...
// // // await this._mcpServer.cleanup();
    ...
    // console.warn(' HTTP MCP Server stopped successfully''
        this.emit('stopped')
        resolve()})});
  //   }
...
/** Get server status */
  ...
  getStatus(): {running = (Date.now() - this._metrics.uptime) / 1000;
    ...
    const healthChecks = ['
      ...
      healthChecks.some(check => check.status === 'error'  ?? check.status === 'offline') ? 'error''degraded'''
