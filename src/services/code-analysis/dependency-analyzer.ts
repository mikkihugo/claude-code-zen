...
// const _cruiserModule = awaitimport('dependency-cruiser');
  ...
  Dependency;
  analysis;
  complete = {fileExtensions = > new RegExp(p.replace('**/', '.*/'))),tsConfig = // await madge(targetPath, madgeConfig);`
          ...
          id = {nodes = graph.nodes.size;
                  ...
                  2 * edgeCount;
                  {
                    ...
                    for (const _dep of dependencies) {
                      '
      relationships.push(id = 2)
                      return 'low'; // if(cycle.length <= 4) return 'medium'; // LINT: unreachable code removed'
                      if (cycle.length <= 6) {
                        return 'high';
                        '
