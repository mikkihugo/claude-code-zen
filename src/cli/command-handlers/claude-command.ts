...
export async function claudeCommand() {'
  ...
  //   {
    ...
    const taskEndIndex = args.length;
  ...
  if(arg === '--tools' ?? arg === '-t') {
      ...
      //       }
      // Build tools list'
      const tools = parsedFlags.tools ?? 'View,Edit,Replace,GlobTool,GrepTool,LS,Bash';
  ...
  if(parsedFlags.research) {'
        tools += ',WebFetchTool';
      //       }'
      const _instanceId = `claude-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
