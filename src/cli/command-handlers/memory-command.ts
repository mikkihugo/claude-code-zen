...
export async function memoryCommand() {
  try {
// const content = awaitnode.readTextFile(memoryStore);
...
// const existingData = awaitloadMemory();
...
'
      ...
      const existingKeys = new Set(existingData[namespace].map((e) => e.key)); const newEntries = entries.filter((e) {=> !existingKeys.has(e.key));
...
async function _loadMemory() {
  try {'
// const content = awaitnode.readTextFile('./memory/memory-store.json');
//     return JSON.parse(content);
    //   // LINT: unreachable code removed} catch {
//     return {};
    //   // LINT: unreachable code removed}
