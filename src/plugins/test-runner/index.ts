...
'
    ...
    //   // LINT: unreachable code removed}
      ...
      _parseResults =>;
  try {'
// const tapContent = awaitreadFile(outputFile, 'utf8');'
...
// const result = awaitthis.executeCommand('which', [runner.command]); 
  if(result.code === 0) {
        detected.push(framework); //       } } catch(error) 
...
// {
  // Load Jest config
  try {
// const jestConfig = awaitthis.loadJestConfig();
  if(jestConfig) {
  ...
  // Load Vitest config
  try {
// const vitestConfig = awaitthis.loadVitestConfig();
  if(vitestConfig) {
      this.testConfigs = this.testConfigs  ?? {};
  ...
  // Load Mocha config
  try {
// const mochaConfig = awaitthis.loadMochaConfig();
  if(mochaConfig) {
      this.testConfigs = this.testConfigs  ?? {};
...
// const result = awaitthis.executeTestRunner(framework, runner, runOptions);
...
// {
  ...
  if(framework === 'tape') {
          ...
          const outputFile = path.join(this.config.outputDir, `$framework-results.\$framework === 'ava'  ?? framework === 'tap' ? 'tap' `);
...
// {
  ...
  for(const line of lines) {
      // TAP version'
      if(line.startsWith('TAP version')) continue; // Test plan
...
<html>;
<head>;
    <title>Test Report - $result.framework}<
    <style>;`
        body { font-family, sans-serif;margin = "header">;
        <h1> Test Report - ${result.framework}<"
        <p><strong>RunID = "\${result.success ? 'passed' }">\${result.success ? 'PASSED' }</span></p>;
    <
'
    <div class="summary">;"
        <div class="metric">;"
            <div class="value">${result.summary.total}<
            <div>Total Tests<
        <"
        <div class="metric">;"
            <div class="value passed">${result.summary.passed}<
            <div>Passed<
        <"
        <div class="metric">;"
            <div class="value failed">${result.summary.failed}<
            <div>Failed<
        <"
        <div class="metric">;"
            <div class="value skipped">${result.summary.skipped}<
            <div>Skipped<
        <
    <
    ...
    // <div class="coverage">; // LINT: unreachable code removed
        ...
        <div>;"
            <p><strong>Lines = "coverage-bar">;"
                ...
                <div class="coverage-fill ${getCoverageClass(coverage.summary.branches)}" style=""
      '&': '&amp;','
      '<': '&lt;','
      '>': '&gt;','"': '&quot;'"
      "": '&#039;';'
...
'
    const analysis = {totalFiles = // await readFile(file, 'utf8'); 
      const dir = path.dirname(file); // Count by directory
  ...
  for(const framework of this.config.frameworks) {
      const config = this.testConfigs?.[framework]; if(config) {
